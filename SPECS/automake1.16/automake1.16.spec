...
./configure \
    --prefix=%{_prefix} \
    --mandir=%{_mandir} \
    --infodir=%{_infodir}/%{name} \
    --bindir=%{_libexecdir}/%{name}/bin \
    --datadir=%{_datadir} \
    --libdir=%{_libdir} \
    --docdir=%{_docdir}/%{name}-%{version} \
    --program-suffix=-%{api_version}
...
pushd %{buildroot}%{_libexecdir}/%{name}/bin
  for i in $(ls); do
    ln -sf %{_libexecdir}/%{name}/bin/$i %{buildroot}%{_bindir}/${i}
    ln -sf $i ./$(echo $i | sed 's/-%{api_version}//g')
  done
popd

mkdir -p %{buildroot}%{_datadir}/Modules/modulefiles/automake
cat > %{buildroot}%{_datadir}/Modules/modulefiles/automake/%{version} <<ENDPY
#%Module
...
module-whatis   "Sets up automake %{version} as default in your environment"

module load m4
module load autoconf
prepend-path PATH "%{_libexecdir}/%{name}/bin"
prepend-path INFOPATH "%{_infodir}/%{name}"
ENDPY
...
%{_mandir}/man1/*
%{_datadir}/Modules/modulefiles/automake/*
